function [h,atab,ctab,outstats,aoc_fig] = aoctool1(x,y,g,alpha,xname,yname,gname,GRP_DIR, displayopt,model)
...
if (strcmp(action, 'start'))
   ...
   dodisp = 1;
   if (nargs >= 9 && isequal(displayopt, 'off')), dodisp = 0; end
...
switch action
 ...
 case 'start',
  ...
  if (dodisp)
     ...
     set(fitline(1), 'color','b')
     set(fitline(2), 'color','r')
     set(dataline(1), 'color','b')
     set(dataline(2), 'color','r','Marker','o')
     ...
     py=plot(xHalfLine,yHalfLine,'k','LineStyle','--');
     set(get(get(py,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');

    filename_fig=strcat(GRP_DIR,'\\Figure_3C');
    print(aoc_fig,'-dtiff','-r300', filename_fig);
    saveas(aoc_fig,strcat(filename_fig,'.eps'));
    saveas(aoc_fig,strcat(filename_fig,'.tif'));
    saveas(aoc_fig,strcat(filename_fig,'.fig'));
...
statbrushlink1(plotHandles.fitline,false);
yrange = get(gca,'YLim');
xrange = get(gca,'XLim');
...
statbrushlink1(reference_line,false);
