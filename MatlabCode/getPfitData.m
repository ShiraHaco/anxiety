...
%       priorLambda = @(x) (x>=0).*(x<=1);
        priorLambda = @(x) (0.1-x).*(x>=0).*(x<=.1);    
%       priorGamma = @(x) (x>=0).*(x<=1);
        options.priors{3} = priorLambda; 
%       options.priors{4} = priorGamma;
        options.borders = nan(5,2);
        options.borders(3,:)=[0,.1];
%       options.borders(4,:)=[0,.01];
        options.sigmoidName = 'norm';
        options.expType = 'equalAsymptote';
        options.stimulusRange =[-8,8];

    result = psignifitt(pfit_input,options);
    result_std = getStandardParameters(result);
    bias=result_std(1);
    thr=result_std(2);
    bias68CI=result.conf_Intervals(1,:,3);
else
    ...
    [devianceResiduals,deviance,samples_deviance,samples_devianceResiduals] = getDeviance(result,'');
    [devianceResiduals_null,deviance_null] = getDeviance_null(result);
    %r_r is the pseudo r square
    r_r=(deviance_null-deviance)/deviance_null;
...
if is_new_pfit % shir for new pfit
    ...
    pfitData.Rsquare=r_r; %need to complete. old code for Rsquare is at the end of this script
else
